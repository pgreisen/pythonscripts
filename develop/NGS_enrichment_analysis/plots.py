...
def volcano_plot(df,xvalue,yvalue, title=None, colors=None, log_bins=True,
                 logp_max=10):
    """
    ...
    *logp_max* sets the maximum minus log-transformed p-value that will be
    ...
    """
    df = df.loc[:, (yvalue, xvalue)]
    df.dropna(axis="index", how="any", inplace=True)
    df[yvalue] = -np.log10(df[yvalue])
    df.loc[df[yvalue] > logp_max, yvalue] = logp_max  # set max
    ...
    if log_bins:
        ...
        bins = None
    hexbin = ax.hexbin(x=df[xvalue].values, y=df[yvalue].values,
    ...
    ax.set_xlim([1.05 * df[xvalue].min(), 1.05 * df[xvalue].max()])
    ax.set_ylim([0., 1.05 * df[yvalue].max()])
