...
class pdbfile:
    ...
    # @requires atom name
    def get_atoms_per_name(self,pdblist, atomname):
        # dict_of_coordinates = {}
        list_with_coordinates = []
        for line in pdblist:
            if str(line[13:15]).strip() == atomname:
                atomindex = str(line[7:11]).strip()
                coordinates = self.get_xyz(line)
                list_with_coordinates.append( coordinates )
    ...
    # @Requires residue - necessary to specify residue name and number
    # which number is specified here
    def get_all_pdbatoms( self, pdblist ):
        list_w_coordinates = []
        for line in pdblist:
            if(line[0:4] == "ATOM" or line[0:4] == "HETA"  ):
                coordinates = self.get_xyz(line)
                list_w_coordinates.append( coordinates )
        return list_w_coordinates
...
# Require file
    # Input crystal coordinates
    def get_t_coordinates(self, coordinates, pdbfile):
        t_coordinates = []
        i = 0
        for line in pdbfile:
            if( line[0:4] == "ATOM" or line[0:4] == "HETA" ):

                x = '%.3f' %coordinates[i][0]
                y = '%.3f' %coordinates[i][1]
                z = '%.3f' %coordinates[i][2]
