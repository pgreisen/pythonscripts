...
@returns a file with the value called chi_table.txt
...
def write_table(outfile, residuename , nr , torsionangles):
    
    angles = ''

    for i in torsionangles:

        angles = angles + str(i) + '\t'
        
    angles = angles + '\n'

    outfile.write(residuename+'\t'+str(nr)+'\t'+angles)
...
def main():
    ...
    outfile = open('chi_table.txt','w')


    outfile.write("Name\tNR\tchi1\tchi2\tchi3\tchi4\tchi5 \n")
    ...
    for i in residue_number:
        ...
        write_table(outfile, residue_name , i , chi_angles)
