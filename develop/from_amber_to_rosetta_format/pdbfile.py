...
class pdbfile:
    ...
    def insert_correct_histidines(self,line):
        print line[21:22]
        if(line[17:19] == "HI"):

            line = str(line[0:17])+"HIS A"+str(line[22:56])+"1"+str(line[57:])

        elif( line[0:4] == "ATOM" and str(line[21:22]) != "A"):
            line = str(line[0:21])+"A"+str(line[22:56])+"1"+str(line[57:])

        return line


    def convert_to_rosetta_format(self,pdbline):
    ...
    def split_into_multiple_pdb_files(self,list_of_lines,delimiter='MODEL'):
        ...
        for line in list_of_lines:
            if(line.startswith( delimiter ) ):
                ...
                pdbfile = []
            if(line[0:4] == 'ATOM'):
                line = self.insert_correct_histidines(line)
                pdbfile.append(line)