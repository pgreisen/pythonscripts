    ...
    def get_scores(self,inputfile,x_string, y_string):
        ...
        for line in input_file:
            ...
            if( line[0:4] == "SEQU" ):
                ...
                tmpline =  line.split()
                if( len(tmpline) <x_index or len(tmpline) < y_index or len(tmpline) < tag_index):
                    continue
        ...
        with open('datafile.csv', 'wb') as csvfile:
            ...
            for index in sorted_by_ystring:
                ...
                if( dummy < self.number_of_pdbs_to_dump ):
                    tmppdb = "/work/greisen/files/extract_pdbs_from_silent_file_binary/bin/extract_atomtree_diffs.static.linuxgccrelease -extra_res_fa $1 -in:file:silent silent.out -in::file::silent_struct_type binary @/work/greisen/files/rescore_w_facts/flags -database /work/greisen/git_rosetta/database -tags "+str(tagstring[index])
