...
class get_union:
    ...
    def main(self):
        ...
        close()
        ####################################################
        # split up plot into additional ranges
        # title("3' Sort 4 Enrichment Apixaban")
        scatter(x2,y2, color='red', s=40)
        xlim(2,4)
        ...
        for label, i, j in zip(labels2, x2, y2):
            annotate(label, size=12, xy = (i, j), xytext = (-20, 20), textcoords = 'offset points', ha = 'right',\
 va = 'bottom', bbox = dict(boxstyle = 'round,pad=0.5', fc = 'gray', alpha = 0.25), arrowprops = dict(arrowstyle \
= '->', connectionstyle = 'arc3,rad=0'))

        savefig(self.prime+"_APN_FEN_APNenriched_index1"+self.round+".eps")
        savefig(self.prime+"_APN_FEN_APNenriched)index1"+self.round+".png")
        close()
...
= '->', connectionstyle = 'arc3,rad=0'))
        ...
        savefig(self.prime+"_APN_FEN_APNenriched_index2"+self.round+".eps")
        savefig(self.prime+"_APN_FEN_APNenriched)index2"+self.round+".png")
        close()
