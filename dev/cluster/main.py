...
class AlignResidues:
    ...
    def __init__(self):
        ...
        # the aligned coordinates will be used for RMSD
        self.queryresiduescoordinates_aligned = {}
    ...
    def get_rmsd(self,t_a,o_a):
        ...
        sum = 0.0
        length = len(t_a)
        assert length == len(o_a)
        for key in t_a:
            tk = key.split('_')
            newkey = tk[0]+'_'+tk[2]
            if newkey in o_a:
                df = ( t_a[key] - o_a[newkey] )
                sum = sum + (df[0]*df[0] + df[1]*df[1] + df[2]*df[2]  )
        sum = sum / length
    ...
    def main(self):
        ...
        t_coordinates = pdbfile_instance.get_transformed_coordinates_pdbfile( query_coor, queryfile, self.queryresiduescoordinates_aligned, self.rmsd )

        # compute rmsd between the newly aligned coordinates and the target object
        rmsd = self.get_rmsd( self.queryresiduescoordinates_aligned, self.targetresidues)

        print input_variables.queryfile, rmsd
