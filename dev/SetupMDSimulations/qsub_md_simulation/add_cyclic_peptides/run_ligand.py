...
quit
'''
    tmp_file = open('generate_apo_parameters.sh','w')
    #########
    tmp_file.write(template)
    tmp_file.close()
...
def main():
    ...
    elif( pdbfile_present == False):
        generate_input_file_for_library_files(parmfile,libfile,ligname)
        generate_parms_for_amber = "/gscratch/baker/greisen/AT12/amber12/bin/tleap -s -f /gscratch/baker/greisen/AT12/amber12/dat/leap/cmd/leaprc.ff12SB -f generate_apo_parameters.sh"
        subprocess.Popen(generate_parms_for_amber,shell=True).wait()
    ...
    if( pdbfile_present == True):
        ...
        sffa.get_template_ligand(ligname,parmfile,libfile)
    ...
    exe_minimization_whole_system="mpiexec.hydra -np 12 /gscratch/baker/greisen/AT12/amber12/bin/pmemd.MPI -O -i minimize_sys.in -p solv.prmtop -c solvent_minimized.rst -r minimization_whole_system_apo.rst"
    ...
    heat_system_exe="mpiexec.hydra -np 12 /gscratch/baker/greisen/AT12/amber12/bin/pmemd.MPI -O -i heat.in -p solv.prmtop -r heat.rst -ref minimization_whole_system_apo.rst -c minimization_whole_system_apo.rst"
