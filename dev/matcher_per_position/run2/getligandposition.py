...
class GetLigandPositions:
    ...
    def get_angle_between_vectors(self, vector1, vector2):
        ...
        """
        dot_p = dot(n1, n2)
        norm_x = math.sqrt((n1 * n1).sum())
        norm_y = math.sqrt((n2 * n2).sum())
    ...
    def get_positions(self, pdbfile, ligand_coor, positions):
        ...
        for line in pdbfile:
            ...
            if( line[0:4] == 'ATOM' and tmp_pos in positions ):
                ...
                if (numpy.linalg.norm(ligand_coor - numpy.array([numpy.float(x), numpy.float(y), numpy.float(
    ...
    def get_protein_positions(self, pdbfile, ligand_coor):
        ...
        for line in pdbfile:
            ...
            if line[0:4] == 'ATOM':
                ...
                if (numpy.linalg.norm(ligand_coor - numpy.array(
                        [numpy.float(x), numpy.float(y), numpy.float(z)])) <= self.DISTANCE_AWAY_FROM_LIGAND):

                        new_positions.append(numpy.str(tmp_pos))
        # temporary fix
        # return sorted(numpy.set(new_positions))
